...
import { layoutClasses, SORT_OPTIONS } from './../assests/dummy';
...
const PendingPage = () => {
  ...
  return (
    <div className={layoutClasses.container}>
      <div className={layoutClasses.headerWrapper}>
        <div>
          ...
          <p className='text-sm text-gray-500 mt-1 ml-7'>
            {sortedPendingTask.length} task  {sortedPendingTask.length !== 1 && 's'} {' '}
      ...
      <div className={`space-y-4`}>
        {sortedPendingTask.length === 0 ? (
          <div className={layoutClasses.emptyState}>
              ...
              <button onClick={()=> setShowModel(true)} className={layoutClasses.emptyBtn}>Create New Task</button>
          </div>
        ):(
          sortedPendingTask.map(tasks => (
            <TaskItem key={tasks._id || tasks.id} task={tasks} showCompleteCheckbox onDelete={() => handleDelete(tasks._id || tasks.id )} onToggleComplete = {() => handleToggleComplete(
              tasks._id || tasks.id,
              t.completed
            )}
            onEdit = {() => {setSelectTask(tasks); setShowModel(true);}}
      ...
      <TaskModel isOpen={!!selectTask || showModal} onClose={() => {setShowModel(false); setSelectTask(null); refreshTasks();}} taskToEdit={setSelectTask}/>
    </div>
  )
}
