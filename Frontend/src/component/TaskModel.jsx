...
const TaskModel = ({ isOpen, onClose, taskToEdit, onSave, onLogout }) => {
  ...
  // ⛔ if modal should not show
  if (!isOpen) return null;

  return (
    <div className='flex inset-0 backdrop-blur-sm bg-black/20 z-50 fixed items-center justify-center p-4'>
      <div className='bg-white border border-purple-100 rounded-xl max-w-md w-full shadow-lg p-6 animate-fadeIn'>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          <div>
            ...
            <textarea name='description' rows="3" onChange={handleChange} value={taskData.description} className={baseControlClasses} placeholder='Add details about your task' />
          </div>
            ...
            <div className='flex gap-4'>
              ...
              { val: 'No', label: 'In Progress' }
                ...
                <label key={val} className='flex items-center'>
                  <input type='radio' name='completed' value={val} checked={taskData.completed === val} onChange={handleChange} className='h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded ' />
