...
const StatCard = ({ title, value, icon }) => (
  <div className='p-2 sm:p-3 rounded-full bg-white shadow-sm border border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-200 group'>
    <div className='flex items-center gap-2'>
      <div className='p-1.5 rounded-lg bg-gradient-to-br from-fuchsia-500/10 to-purple-500/10 group-hover:bg-fuchsia-500/20 group-hover:to-purple-500/20'>
        {icon}
      </div>
...
const Layout = ({ onLogout, user }) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      if (!token) throw new Error("Sorry! No auth token found");

      const { data } = await axios.get("https://task-manager-edbm.onrender.com/api/tasks/gp", {
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 max-w-md">
          ...
          <button
            onClick={fetchTasks}
            className="mt-4 py-2 px-4 bg-red-700 text-white rounded-lg text-sm font-medium hover:bg-red-800 transition-colors"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Sidebar user={user} tasks={tasks} loading={loading} error={error} />
              ...
              <div className='space-y-2 sm:space-y-3'>
                ...
                {tasks.length === 0 && (
                  <div className='text-center py-4 sm:py-6 px-2'>
                    ...
                    <p className='text-sm text-gray-500'>No recent task</p>
                    <p className='text-xs text-gray-400 mt-1'>Task will appear here</p>
                  </div>
                )}
              </div>
            </div>
