...
import { getPriorityColor, MENU_OPTIONS, TI_CLASSES } from './../assests/dummy';
...
const TaskItem = ({ task, onRefresh, onLogout, showCompleteCheckbox = true }) => {
  ...
  const progress = subTask.length
    ? (subTask.filter((st) => st.completed).length / subTask.length) * 100
    : 0;
  ...
  return (
    <>
      ...
      <TaskModel isOpen={showEditModal} onClose={() => setShowEditModal(false)} taskToEdit={task} onSave={handleSave}/>
    </>
  );
};
